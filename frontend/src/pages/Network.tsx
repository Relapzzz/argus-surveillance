...
export default function Network() {
  ...
  const select = (id: string) => {
    ...
    setParams(previous => { previous.set('entity', id); return previous })
    setTab('entity')
  }
  ...
  const selectedLabel = selected && nodes.find(n => n.id === selected)?.label
  ...
  return <div className="stage">
    <section className="stage-main">
      ...
      {graph && <footer className="stage-foot"><span className="mono">{graph.nodes.length} entities · {graph.edges.length} relationships</span>
        <span className="status" role="status">{path ? `${path.node_ids.length} entities highlighted · ${path.edge_ids.length} hops` : highlights.length ? `${highlights.length} entities highlighted` : selectedLabel ? <>Selected: <b>{selectedLabel}</b></> : 'Click an entity to inspect it. Scroll to zoom, drag to pan.'}</span>
    ...
    <aside className="inspector" aria-label="Inspector">
      ...
      <div className="inspector-body">
        ...
        {tab === 'route' && <PathFinder nodes={nodes} selected={selected} onClear={() => setPath(undefined)} onPath={result => { setFocused(undefined); showAll(); setPath(result); setParams(previous => { previous.delete('highlight'); return previous }) }} />}
