...
def new_officer(rng, used_names, rank, station):
    ...
    return Person(
        ...
        vehicle=None, prior_case_count=0, gang=None, role="officer", rank=rank, station=station,
    )
...
def build_universe(rng, used_names, used_aliases, used_phones, used_accounts, used_plates):
    ...
    officer_stations = rng.sample(list(POLICE_STATION_BY_AREA.values()), 4)
    officers = [
        new_officer(rng, used_names, rng.choice(["PSI", "API"]), officer_stations[i])
        for i in range(4)
    ]
...
EXTORTION_TEMPLATES = [
    {"text": "The complainant states that on {date} at about {itime} hrs near {location} the accused, identified as {accused_clause}, stopped {pronoun} and demanded Rs. {amount}{phone_clause}. The complainant was threatened with death if the amount was not paid. Fearing for {poss} life, the complainant approached the police station to lodge a report. Investigation has been taken up.{burner_clause}", "sections": "308(2), 351(3)"},
    {"text": "On {date} at about {itime} hrs, the complainant received a visit from {accused_clause}, near {location}, who demanded Rs. {amount} failing which the complainant's shop would be shut down{phone_clause}. Threats to damage the shop were repeated over the following days. The complainant, unable to bear the pressure, reported the matter to the police. A case has been registered.{burner_clause}", "sections": "308(2), 351(2)"},
    {"text": "It is stated that the complainant was contacted near {location} on {date} at about {itime} hrs by {accused_clause}, who demanded a sum of Rs. {amount} for allowing the complainant's business to continue{phone_clause}. On refusal, threats to kill members of the complainant's family were issued. The complainant reported the extortion attempt the same evening.{burner_clause}", "sections": "308(2), 351(3)"},
    ...
    {"text": "The complainant states that {accused_clause}, along with associates, cornered {pronoun} near {location} on {date} at about {itime} hrs and demanded Rs. {amount}{phone_clause}. When the complainant expressed inability to pay, {subj} was threatened with grievous physical harm. The complainant lodged the report soon after the incident. Investigation has been initiated.{burner_clause}", "sections": "308(2), 351(3)"},
    ...
    {"text": "The complainant narrates that {accused_clause}, met {pronoun} near {location} on {date} at about {itime} hrs and demanded Rs. {amount} for settling a fabricated claim{phone_clause}. Threats to kill the complainant's family were made in case of non-payment. The complainant approached the police station the same day to report the matter. The investigation has been taken up accordingly.{burner_clause}", "sections": "308(2), 351(3)"},
]

ASSAULT_TEMPLATES = [
    {"text": "The complainant states that on {date} at about {itime} hrs near {location}, {accused_clause}, assaulted {pronoun} following a heated argument{phone_clause}. The complainant sustained injuries and was taken to a nearby hospital for treatment. The accused warned the complainant against reporting the matter to the police. Passersby intervened and separated the parties. The complainant reported the matter to the police thereafter.{burner_clause}", "sections": "115(2), 351(2)"},
    {"text": "On {date} at about {itime} hrs, the complainant was walking near {location} when {accused_clause}, blocked {poss} path and started beating {pronoun} over a previous dispute{phone_clause}. The complainant suffered injuries to {poss} head and hands. Neighbours rushed to help and the accused fled the spot. The complainant lodged the report soon after.{burner_clause}", "sections": "115(2)"},
    {"text": "It is stated that {accused_clause}, attacked the complainant near {location} on {date} at about {itime} hrs using a blunt weapon{phone_clause}. The complainant sustained grievous injuries and was rushed to hospital. The accused threatened to kill the complainant if the matter was reported to the police. Family members present at the scene identified the accused. A case has been registered for the assault.{burner_clause}", "sections": "117(2), 351(3)"},
...
def build_incidents(rng, universe, to_reveal):
    ...
    for crime_type, gang, count in plan:
        ...
        for local_idx in range(count):
            ...
            police_station = complainant.station if crime_type == "drug" else POLICE_STATION_BY_AREA[location]
