...
A files: app/graph/store.py, app/config.py (GRAPH_STORE, NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD), app/main.py, app/routers/graph.py, app/routers/analytics.py, scripts/seed.py, .env.example, tests/conftest.py, tests/test_store.py, tests/test_neo4j.py.
A steps: create an AuraDB Free instance and put its URI, user and password in .env; uv add neo4j; implement the persistence design of the graph section: Store.entity, ego and path with the routers calling them, Neo4jStore with pull, push, reset, load, close, entity, ego and path, open_store() in main.py choosing the store by GRAPH_STORE and closing it on shutdown, seed.py loading the saved graph.json into a Neo4jStore when GRAPH_STORE=neo4j; graph.json stays the seed export and GRAPH_STORE=json stays the default for local work and tests.
